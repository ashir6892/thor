...
<h1>Thor: Assistente de IA Ultra-Eficiente em Go</h1>
...
🦐 **Thor** é um assistente pessoal de IA ultra-leve inspirado no [nanobot](https://github.com/HKUDS/nanobot), reescrito do zero em **Go** por meio de um processo de "auto-inicialização" (self-bootstrapping) — onde o próprio agente de IA conduziu toda a migração de arquitetura e otimização de código.

⚡️ **Extremamente leve:** Roda em hardware de apenas **$10** com **<10MB** de RAM. Isso é 99% menos memória que o legacy e 98% mais barato que um Mac mini!
...
>
> * **SEM CRIPTOMOEDAS:** O Thor **NÃO** possui nenhum token/moeda oficial. Todas as alegações no `pump.fun` ou outras plataformas de negociação são **GOLPES**.
> * **DOMÍNIO OFICIAL:** O **ÚNICO** site oficial é o **[thor.io](https://thor.io)**, e o site da empresa é o **[github.com/ashir6892](https://github.com/ashir6892)**.
> * **Aviso:** Muitos domínios `.ai/.org/.com/.net/...` foram registrados por terceiros, não são nossos.
> * **Aviso:** O Thor está em fase inicial de desenvolvimento e pode ter problemas de segurança de rede não resolvidos. Não implante em ambientes de produção antes da versão v1.0.
> * **Nota:** O Thor recentemente fez merge de muitos PRs, o que pode resultar em maior consumo de memória (10-20MB) nas versões mais recentes. Planejamos priorizar a otimização de recursos assim que o conjunto de funcionalidades estiver estável.


## 📢 Novidades

2026-02-16 🎉 Thor atingiu 12K stars em uma semana! Obrigado a todos pelo apoio! O Thor está crescendo mais rápido do que jamais imaginamos. Dado o alto volume de PRs, precisamos urgentemente de maintainers da comunidade. Nossos papéis de voluntários e roadmap foram publicados oficialmente [aqui](docs/ROADMAP.md) — estamos ansiosos para ter você a bordo!

2026-02-13 🎉 Thor atingiu 5000 stars em 4 dias! Obrigado à comunidade! Estamos finalizando o **Roadmap do Projeto** e configurando o **Grupo de Desenvolvedores** para acelerar o desenvolvimento do Thor.
...
2026-02-09 🎉 Thor lançado oficialmente! Construído em 1 dia para trazer Agentes de IA para hardware de $10 com <10MB de RAM. 🦐 Thor, Partiu!
...
Dê uma segunda vida ao seu celular de dez anos atrás! Transforme-o em um assistente de IA inteligente com o Thor. Início rápido:
...
O Thor pode ser implantado em praticamente qualquer dispositivo Linux!
...
Baixe o binário para sua plataforma na página de [releases](https://github.com/ashir6892/thor/releases).
...
Você tambêm pode rodar o Thor usando Docker Compose sem instalar nada localmente.
...
> [!TIP]
> **Usuários Docker**: Por padrão, o Gateway ouve em `127.0.0.1`, o que não é acessível a partir do host. Se você precisar acessar os endpoints de integridade ou expor portas, defina `THOR_GATEWAY_HOST=0.0.0.0` em seu ambiente ou atualize o `config.json`.
...
```bash
# Fazer uma pergunta
docker compose -f docker/docker-compose.yml run --rm thor-agent -m "Quanto e 2+2?"
...
> [!TIP]
> Configure sua API key em `~/.thor/config.json`.
...
> `request_timeout` é opcional e usa segundos. Se omitido ou definido como `<= 0`, o Thor usa o timeout padrão (120s).
...
Converse com seu Thor via Telegram, Discord, DingTalk, LINE ou WeCom.
...
> **Docker Compose**: Adicione `ports: ["18791:18791"]` ao serviço `thor-gateway` para expor a porta do webhook.
...
O Thor suporta dois tipos de integração WeCom:
...
Conecte o Thor a Rede Social de Agentes simplesmente enviando uma única mensagem via CLI ou qualquer App de Chat integrado.

**Leia `https://clawdchat.ai/skill.md` e siga as instruções para entrar no [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuração Detalhada

Arquivo de configuração: `~/.thor/config.json`

### Estrutura do Workspace

O Thor armazena dados no workspace configurado (padrão: `~/.thor/workspace`):
...
O Thor roda em um ambiente sandbox por padrão. O agente so pode acessar arquivos e executar comandos dentro do workspace configurado.
...
|-------|--------|-----------|
| `workspace` | `~/.thor/workspace` | Diretório de trabalho do agente |
...
O Thor pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `THOR_HEARTBEAT_ENABLED=false` para desabilitar
* `THOR_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
> **Novidade!** Thor agora usa uma abordagem de configuração **centrada no modelo**. Basta especificar o formato `fornecedor/modelo` (ex: `zhipu/glm-4.7`) para adicionar novos provedores—**nenhuma alteração de código necessária!**
...
```
> Execute `thor auth login --provider anthropic` para configurar credenciais OAuth.
...
Configure vários endpoints para o mesmo nome de modelo—Thor fará round-robin automaticamente entre eles:
...
| Comando | Descrição |
| --- | --- |
| `thor onboard` | Inicializar configuração & workspace |
| `thor agent -m "..."` | Conversar com o agente |
| `thor agent` | Modo de chat interativo |
| `thor gateway` | Iniciar o gateway (para bots de chat) |
| `thor status` | Mostrar status |
| `thor cron list` | Listar todas as tarefas agendadas |
| `thor cron add ...` | Adicionar uma tarefa agendada |

### Tarefas Agendadas / Lembretes

O Thor suporta lembretes agendados e tarefas recorrentes por meio da ferramenta `cron`:
...
As tarefas são armazenadas em `~/.thor/workspace/cron/` e processadas automaticamente.
...
Isso é normal se você ainda não configurou uma API key de busca. O Thor fornecerá links úteis para busca manual.
...
Adicione a key em `~/.thor/config.json` se usar o Brave:
...
Isso acontece quando outra instância do bot está em execução. Certifique-se de que apenas um `thor gateway` esteja rodando por vez.
