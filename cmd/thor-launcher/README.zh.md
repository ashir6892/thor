...
Thor 的独立启动器，提供可视化 JSON 配置编辑和 OAuth Provider 认证管理。
...
```
Usage: thor-launcher [options] [config.json]

Arguments:
  config.json    配置文件路径（默认: ~/.thor/config.json）
...
认证完成后 Google 回调至 `GET /auth/callback`，自动保存凭据并重定向回 thor-config 页面。
