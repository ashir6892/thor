...
func TestGetConfig_MissingFile_ReturnsDefault(t *testing.T) {
	mux := http.NewServeMux()
	RegisterConfigAPI(mux, "/tmp/nonexistent-thor-launcher-test/config.json")
...
func TestDefaultConfigPath(t *testing.T) {
	...
	if !strings.HasSuffix(path, filepath.Join(".thor", "config.json")) {
		t.Errorf("expected path ending with .thor/config.json, got %q", path)
	}
}
