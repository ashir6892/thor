...
A standalone launcher for Thor, providing visual JSON editing and OAuth provider authentication management.
...
```bash
# Build
go build -o thor-launcher ./cmd/thor-launcher/

# Run with default config path (~/.thor/config.json)
./thor-launcher

# Specify a config file
./thor-launcher ./config.json

# Allow LAN access
./thor-launcher -public
```
...
```
Usage: thor-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.thor/config.json)
...
{
  "config": { ... },
  "path": "/Users/xiao/.thor/config.json"
}
```
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the thor-config UI.
...
Gets the running status of the `thor gateway` process.
...
Starts the `thor gateway` process in the background.
...
Stops the running `thor gateway` process.
