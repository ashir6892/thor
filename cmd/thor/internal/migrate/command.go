...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from legacy config to thor",
		Args:  cobra.NoArgs,
		Example: `  thor migrate
  thor migrate --from legacy
  thor migrate --dry-run
  thor migrate --refresh
  thor migrate --force`,
	...
	cmd.Flags().StringVar(&opts.Source, "from", "legacy",
		"Source to migrate from (e.g., legacy)")
	cmd.Flags().BoolVar(&opts.Refresh, "refresh", false,
		"Re-sync workspace files from legacy source (repeatable)")
	...
	cmd.Flags().StringVar(&opts.SourceHome, "source-home", "",
		"Override source home directory (default: ~/.legacy)")
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.thor)")
