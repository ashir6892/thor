...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					thorconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model thorconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []thorconfig.ModelConfig) {
...
func isModelValid(model thorconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *thorconfig.ModelConfig) {
	if model == nil {
		return
	}
