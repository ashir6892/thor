...
import (
	...
	configstore "thor/cmd/thor-launcher-tui/internal/config"
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open thor agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
