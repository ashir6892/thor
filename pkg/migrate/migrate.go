...
import (
	...
	"thor/pkg/migrate/sources/legacy"
)
...
func NewMigrateInstance(opts Options) *MigrateInstance {
	...
	legacy_handler, err := legacy.NewLegacyHandler(opts)
	if err == nil {
		instance.Register(legacy_handler.GetSourceName(), legacy_handler)
	}
...
func (m *MigrateInstance) Run(opts Options) (*Result, error) {
	...
	fmt.Println("Migrating from Source to Thor")
	fmt.Printf("  Source:      %s\n", sourceHome)
	fmt.Printf("  Target: %s\n", targetHome)
	fmt.Println()
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert Source config to Thor format",
			})
		}
	}
