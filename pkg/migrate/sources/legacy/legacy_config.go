...
type LegacyConfig struct {
	...
	Models   *LegacyModels   `json:"models"`
	Agents   *LegacyAgents   `json:"agents"`
	Tools    *LegacyTools    `json:"tools"`
	Channels *LegacyChannels `json:"channels"`
	Cron     json.RawMessage   `json:"cron"`
	Hooks    json.RawMessage   `json:"hooks"`
	Skills   *LegacySkills   `json:"skills"`
...
type LegacyAgents struct {
	Defaults *LegacyAgentDefaults `json:"defaults"`
	List     []LegacyAgentEntry   `json:"list"`
}

type LegacyAgentDefaults struct {
	Model     *LegacyAgentModel `json:"model"`
	Workspace *string             `json:"workspace"`
	Tools     *LegacyAgentTools `json:"tools"`
	Identity  *string             `json:"identity"`
}

type LegacyAgentModel struct {
	Simple    string   `json:"-"`
	Primary   *string  `json:"primary"`
	Fallbacks []string `json:"fallbacks"`
}

func (m *LegacyAgentModel) GetPrimary() string {
	if m.Simple != "" {
		return m.Simple
	}
	if m.Primary != nil {
...
func (m *LegacyAgentModel) GetFallbacks() []string {
	return m.Fallbacks
}

type LegacyAgentEntry struct {
	ID        string              `json:"id"`
	Name      *string             `json:"name"`
	Model     *LegacyAgentModel `json:"model"`
	Tools     *LegacyAgentTools `json:"tools"`
...
type LegacyChannels struct {
	Telegram    *LegacyTelegramConfig    `json:"telegram"`
	Discord     *LegacyDiscordConfig     `json:"discord"`
	Slack       *LegacySlackConfig       `json:"slack"`
	WhatsApp    *LegacyWhatsAppConfig    `json:"whatsapp"`
	Signal      *LegacySignalConfig      `json:"signal"`
	Matrix      *LegacyMatrixConfig      `json:"matrix"`
	GoogleChat  *LegacyGoogleChatConfig  `json:"googlechat"`
	Teams       *LegacyTeamsConfig       `json:"msteams"`
	IRC         *LegacyIrcConfig         `json:"irc"`
	Mattermost  *LegacyMattermostConfig  `json:"mattermost"`
	Feishu      *LegacyFeishuConfig      `json:"feishu"`
	IMessage    *LegacyIMessageConfig    `json:"imessage"`
	BlueBubbles *LegacyBlueBubblesConfig `json:"bluebubbles"`
	QQ          *LegacyQQConfig          `json:"qq"`
	DingTalk    *LegacyDingTalkConfig    `json:"dingtalk"`
	MaixCam     *LegacyMaixCamConfig     `json:"maixcam"`
}
...
type LegacyDiscordConfig struct {
...
type LegacyWhatsAppConfig struct {
...
type LegacySignalConfig struct {
...
type LegacyMatrixConfig struct {
...
type LegacyGoogleChatConfig struct {
...
type LegacyMattermostConfig struct {
	BotToken  *string  `json:"botToken"`
	BaseURL   *string  `json:"baseUrl"`
	DmPolicy  *string  `json:"dmPolicy"`
	AllowFrom []string `json:"allowFrom"`
...
type LegacyFeishuConfig struct {
	AppID             *string  `json:"appId"`
...
type LegacyIMessageConfig struct {
...
type LegacyBlueBubblesConfig struct {
...
type LegacyMaixCamConfig struct {
	Host      *string  `json:"host"`
	Port      *int     `json:"port"`
	DmPolicy  *string  `json:"dmPolicy"`
	AllowFrom []string `json:"allowFrom"`
...
func (c *LegacyConfig) GetEnabled() bool {
	return true
}
...
func LoadLegacyConfigFromDir(dir string) (*LegacyConfig, error) {
	candidates := []string{
...
func GetProviderConfig(models *LegacyModels) map[string]LegacyProviderConfig {
	result := make(map[string]LegacyProviderConfig)
	if models == nil || models.Providers == nil {
		return result
	}

	for name, raw := range models.Providers {
		var prov LegacyProviderConfig
...
func (c *LegacyConfig) IsChannelEnabled(name string) bool {
	switch name {
	case "telegram":
...
func GetChannelAllowFrom(ch any) []string {
	switch c := ch.(type) {
	case *LegacyTelegramConfig:
		if c == nil {
			return nil
		}
		return c.AllowFrom
	case *LegacyDiscordConfig:
		if c == nil {
			return nil
		}
		return c.AllowFrom
	case *LegacySlackConfig:
		if c == nil {
			return nil
		}
		return c.AllowFrom
	case *LegacyWhatsAppConfig:
		if c == nil {
			return nil
		}
		return c.AllowFrom
	case *LegacyFeishuConfig:
		if c == nil {
			return nil
		}
		return c.AllowFrom
...
func (c *LegacyConfig) GetDefaultModel() (provider, model string) {
...
func (c *LegacyConfig) GetDefaultWorkspace() string {
...
func (c *LegacyConfig) GetAgents() []LegacyAgentEntry {
	if c.Agents == nil {
		return nil
	}
...
func (c *LegacyConfig) HasSkills() bool {
	return c.Skills != nil && c.Skills.Entries != nil && len(c.Skills.Entries) > 0
}

func (c *LegacyConfig) HasMemory() bool {
	return c.Memory != nil && len(c.Memory) > 0
}

func (c *LegacyConfig) HasCron() bool {
	return c.Cron != nil && len(c.Cron) > 0
}

func (c *LegacyConfig) HasHooks() bool {
	return c.Hooks != nil && len(c.Hooks) > 0
}

func (c *LegacyConfig) HasSession() bool {
	return c.Session != nil && len(c.Session) > 0
}

func (c *LegacyConfig) HasAuthProfiles() bool {
	return c.Auth != nil && c.Auth.Profiles != nil && len(c.Auth.Profiles) > 0
}

func (c *LegacyConfig) ConvertToThor(sourceHome string) (*ThorConfig, []string, error) {
	cfg := &ThorConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via thor CLI",
				len(c.Skills.Entries),
			),
		)
	}
	if c.HasMemory() {
		warnings = append(warnings, "Memory backend config not migrated - Thor uses SQLite with vector embeddings")
	}
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in Thor - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in Thor - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - Thor uses per-agent sessions by default")
	}
...
func (c *LegacyConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No Thor adapter available")
	}
	if c.Channels.Matrix != nil {
		*warnings = append(*warnings, "Channel 'matrix': No Thor adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No Thor adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No Thor adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No Thor adapter available - consider iMessage instead",
		)
	}
...
func (c *LegacyConfig) convertAgents(warnings *[]string) []AgentConfig {
...
func (c *ThorConfig) ToStandardConfig() *config.Config {
