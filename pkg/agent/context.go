...
func getGlobalConfigDir() string {
	...
	return filepath.Join(home, ".thor")
}

// Memory returns the MemoryStore for this context builder.
...
func (cb *ContextBuilder) getIdentity() string {
	...
	return fmt.Sprintf(`# thor 🦞

You are thor, a helpful AI assistant.

## Workspace
Your workspace is at: %s
- Memory: %s/memory/MEMORY.md
- Recent Log: %s/memory/RECENT.md (rolling last 10 exchanges — update after EVERY response)
...
4. **Recent Log** - After EVERY response, append the exchange to %s/memory/RECENT.md using the append_file tool. Format each entry as:
   **Time:** YYYY-MM-DD HH:MM
   **User:** <brief summary of user message, max 300 chars>
...
func (cb *ContextBuilder) sourcePaths() []string {
	return []string{
		...
		filepath.Join(cb.workspace, "memory", "RECENT.md"),
	}
}
