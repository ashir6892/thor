...
Thor's tools configuration is located in the `tools` field of `config.json`.
...
By default, Thor blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `THOR_TOOLS_<SECTION>_<KEY>`:

For example:
- `THOR_TOOLS_WEB_BRAVE_ENABLED=true`
- `THOR_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `THOR_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
