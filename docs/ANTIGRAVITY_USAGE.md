# Using Antigravity Provider in Thor

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in Thor.
...
```bash
thor auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where Thor is waiting.

Thor will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.thor/config.json` or override it via the CLI:

```bash
# Override for a single command
thor agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `THOR_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.thor/auth.json user@your-server:~/.thor/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. Thor will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `thor auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
