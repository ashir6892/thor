...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Thor.
...
10. [Creating a New Provider in Thor](#creating-a-new-provider-in-thor)
...
const CLIENT_ID = decode(
  "YOUR_GOOGLE_CLIENT_ID_BASE64_HERE"
);
const CLIENT_SECRET = decode("YOUR_GOOGLE_CLIENT_SECRET_BASE64_HERE");
```
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<ThorConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- Thor codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.thor/auth.json`:
...
## Creating a New Provider in Thor

Thor providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/thor/cmd_auth.go`:
...
thor auth login --provider your-provider
...
thor agent -m "Hello" --model your-model
```
...
export THOR_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export THOR_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `thor auth login --provider antigravity`
...
- Check auth profile storage: `~/.thor/auth.json`
- Re-run `thor auth login --provider antigravity`
