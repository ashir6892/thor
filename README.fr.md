<div align="center">
  <img src="assets/logo.jpg" alt="Thor" width="512">

  <h1>Thor : Assistant IA Ultra-Efficace en Go</h1>
  ...
  <p>
    ...
    <br>
    <a href="https://thor.io"><img src="https://img.shields.io/badge/Website-thor.io-blue?style=flat&logo=google-chrome&logoColor=white" alt="Website"></a>
    <a href="https://github.com/ashir6892"><img src="https://img.shields.io/badge/X_(Twitter)-ashir6892-black?style=flat&logo=x&logoColor=white" alt="Twitter"></a>
  </p>
...
🦐 **Thor** est un assistant personnel IA ultra-léger inspiré de [nanobot](https://github.com/HKUDS/nanobot), entièrement réécrit en **Go** via un processus d'auto-amorçage (self-bootstrapping) — où l'agent IA lui-même a piloté l'intégralité de la migration architecturale et de l'optimisation du code.

⚡️ **Extrêmement léger :** Fonctionne sur du matériel à seulement **10$** avec **<10 Mo** de RAM. C'est 99% de mémoire en moins qu'legacy et 98% moins cher qu'un Mac mini !

<table align="center">
  <tr align="center">
    <td align="center" valign="top">
      <p align="center">
        <img src="assets/thor_mem.gif" width="360" height="240">
      </p>
    </td>
    <td align="center" valign="top">
      <p align="center">
...
>
> * **PAS DE CRYPTO :** Thor n'a **AUCUN** token/jeton officiel. Toute annonce sur `pump.fun` ou d'autres plateformes de trading est une **ARNAQUE**.
> * **DOMAINE OFFICIEL :** Le **SEUL** site officiel est **[thor.io](https://thor.io)**, et le site de l'entreprise est **[github.com/ashir6892](https://github.com/ashir6892)**.
> * **Attention :** De nombreux domaines `.ai/.org/.com/.net/...` sont enregistrés par des tiers et ne nous appartiennent pas.
> * **Attention :** Thor est en phase de développement précoce et peut présenter des problèmes de sécurité réseau non résolus. Ne déployez pas en environnement de production avant la version v1.0.
> * **Note :** Thor a récemment fusionné de nombreuses PR, ce qui peut entraîner une empreinte mémoire plus importante (10–20 Mo) dans les dernières versions. Nous prévoyons de prioriser l'optimisation des ressources dès que l'ensemble des fonctionnalités sera stabilisé.


## 📢 Actualités

2026-02-16 🎉 Thor a atteint 12K étoiles en une semaine ! Merci à tous pour votre soutien ! Thor grandit plus vite que nous ne l'avions jamais imaginé. Vu le volume élevé de PR, nous avons un besoin urgent de mainteneurs communautaires. Nos rôles de bénévoles et notre feuille de route sont officiellement publiés [ici](docs/ROADMAP.md) — nous avons hâte de vous accueillir !

2026-02-13 🎉 Thor a atteint 5000 étoiles en 4 jours ! Merci à la communauté ! Nous finalisons la **Feuille de Route du Projet** et mettons en place le **Groupe de Développeurs** pour accélérer le développement de Thor.
...
2026-02-09 🎉 Thor est lancé ! Construit en 1 jour pour apporter les Agents IA au matériel à 10$ avec <10 Mo de RAM. 🦐 Thor, c'est parti !
...
<img src="assets/compare.jpg" alt="Thor" width="512">
...
<table align="center">
  ...
  <tr>
    <td align="center"><p align="center"><img src="assets/thor_code.gif" width="240" height="180"></p></td>
    <td align="center"><p align="center"><img src="assets/thor_memory.gif" width="240" height="180"></p></td>
    <td align="center"><p align="center"><img src="assets/thor_search.gif" width="240" height="180"></p></td>
  </tr>
  <tr>
...
Donnez une seconde vie à votre téléphone d'il y a dix ans ! Transformez-le en assistant IA intelligent avec Thor. Démarrage rapide :
...
wget https://github.com/ashir6892/thor/releases/download/v0.1.1/thor-linux-arm64
chmod +x thor-linux-arm64
pkg install proot
termux-chroot ./thor-linux-arm64 onboard
```

Puis suivez les instructions de la section « Démarrage Rapide » pour terminer la configuration !

<img src="assets/termux.jpg" alt="Thor" width="512">

### 🐜 Déploiement Innovant à Faible Empreinte

Thor peut être déployé sur pratiquement n'importe quel appareil Linux !
...
Téléchargez le binaire pour votre plateforme depuis la page des [releases](https://github.com/ashir6892/thor/releases).
...
Vous pouvez également exécuter Thor avec Docker Compose sans rien installer localement.
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous avez besoin d'accéder aux endpoints de santé ou d'exposer des ports, définissez `THOR_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.

```bash
# 5. Voir les logs
docker compose -f docker/docker-compose.yml logs -f thor-gateway
...
```bash
# Poser une question
docker compose -f docker/docker-compose.yml run --rm thor-agent -m "Combien font 2+2 ?"

# Mode interactif
docker compose -f docker/docker-compose.yml run --rm thor-agent
```
...
> [!TIP]
> Configurez votre clé API dans `~/.thor/config.json`.
...
> `request_timeout` est optionnel et s'exprime en secondes. S'il est omis ou défini à `<= 0`, Thor utilise le délai d'expiration par défaut (120s).
...
Discutez avec votre Thor via Telegram, Discord, DingTalk, LINE ou WeCom
...
> **Docker Compose** : Ajoutez `ports: ["18791:18791"]` au service `thor-gateway` pour exposer le port du webhook.
...
Thor prend en charge deux types d'intégration WeCom :
...
Connectez Thor au Réseau Social d'Agents simplement en envoyant un seul message via le CLI ou n'importe quelle application de chat intégrée.

**Lisez `https://clawdchat.ai/skill.md` et suivez les instructions pour rejoindre [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuration

Fichier de configuration : `~/.thor/config.json`

### Structure du Workspace

Thor stocke les données dans votre workspace configuré (par défaut : `~/.thor/workspace`) :
...
Thor s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes qu'au sein du workspace configuré.
...
|--------|------------|-------------|
| `workspace` | `~/.thor/workspace` | Répertoire de travail de l'agent |
...
```bash
export THOR_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
Thor peut exécuter des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `THOR_HEARTBEAT_ENABLED=false` pour désactiver
* `THOR_HEARTBEAT_INTERVAL=60` pour modifier l'intervalle
...
```bash
thor agent -m "Bonjour, comment ça va ?"
```
...
> **Nouveau !** Thor utilise désormais une approche de configuration **centrée sur le modèle**. Spécifiez simplement le format `fournisseur/modèle` (par exemple, `zhipu/glm-4.7`) pour ajouter de nouveaux fournisseurs—**aucune modification de code requise !**
...
```
> Exécutez `thor auth login --provider anthropic` pour configurer les identifiants OAuth.
...
Configurez plusieurs points de terminaison pour le même nom de modèle—Thor utilisera automatiquement le round-robin entre eux :
...
| ------------------------- | ------------------------------------- |
| `thor onboard`        | Initialiser la configuration & le workspace |
| `thor agent -m "..."` | Discuter avec l'agent                 |
| `thor agent`          | Mode de discussion interactif         |
| `thor gateway`        | Démarrer la passerelle                |
| `thor status`         | Afficher le statut                    |
| `thor cron list`      | Lister toutes les tâches planifiées   |
| `thor cron add ...`   | Ajouter une tâche planifiée           |

### Tâches Planifiées / Rappels

Thor prend en charge les rappels planifiés et les tâches récurrentes via l'outil `cron` :
...
Les tâches sont stockées dans `~/.thor/workspace/cron/` et traitées automatiquement.
...
<img src="assets/wechat.png" alt="Thor" width="512">

## 🐛 Dépannage

### La recherche web affiche « API 配置问题 »

C'est normal si vous n'avez pas encore configuré de clé API de recherche. Thor fournira des liens utiles pour la recherche manuelle.
...
Ajoutez la clé dans `~/.thor/config.json` si vous utilisez Brave :
...
Cela se produit lorsqu'une autre instance du bot est en cours d'exécution. Assurez-vous qu'un seul `thor gateway` fonctionne à la fois.
