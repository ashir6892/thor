...
<h1>Thor: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 Thor は [nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。

⚡️ $10 のハードウェアで 10MB 未満の RAM で動作：legacy より 99% 少ないメモリ、Mac mini より 98% 安い！
...
## 📢 ニュース
2026-02-09 🎉 Thor リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 行くぜ、シャコ！
...
### 🐜 革新的な省フットプリントデプロイ
Thor はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/ashir6892/thor/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに Thor を実行できます。
...
> [!TIP]
> **Docker ユーザー**: デフォルトでは、Gateway は `127.0.0.1` でリッスンしており、ホストからアクセスできません。ヘルスチェックエンドポイントにアクセスしたり、ポートを公開したりする必要がある場合は、環境変数で `THOR_GATEWAY_HOST=0.0.0.0` を設定するか、`config.json` を更新してください。
...
```bash
# 質問を投げる
docker compose -f docker/docker-compose.yml run --rm thor-agent -m "What is 2+2?"
...
> [!TIP]
> `~/.thor/config.json` に API キーを設定してください。
...
> `request_timeout` は任意の秒単位設定です。省略または `<= 0` の場合、Thor はデフォルトのタイムアウト（120秒）を使用します。
...
Telegram、Discord、QQ、DingTalk、LINE、WeCom で Thor と会話できます
...
> **Docker Compose**: `thor-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。
...
設定ファイル: `~/.thor/config.json`

### ワークスペース構成

Thor は設定されたワークスペース（デフォルト: `~/.thor/workspace`）にデータを保存します：
...
Thor はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.thor/workspace` | エージェントの作業ディレクトリ |
...
Thor は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `THOR_HEARTBEAT_ENABLED=false` で無効化
- `THOR_HEARTBEAT_INTERVAL=60` で間隔変更
...
> **新機能！** Thor は現在 **モデル中心** の設定アプローチを採用しています。`ベンダー/モデル` 形式（例: `zhipu/glm-4.7`）を指定するだけで、新しいプロバイダーを追加できます—**コードの変更は一切不要！**
...
```
> OAuth認証を設定するには、`thor auth login --provider anthropic` を実行してください。
...
同じモデル名で複数のエンドポイントを設定すると、Thor が自動的にラウンドロビンで分散します：
...
| コマンド | 説明 |
|---------|------|
| `thor onboard` | 設定＆ワークスペースの初期化 |
| `thor agent -m "..."` | エージェントとチャット |
| `thor agent` | インタラクティブチャットモード |
| `thor gateway` | ゲートウェイを起動 |
| `thor status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。Thor は手動検索用の便利なリンクを提供します。
...
別のインスタンスが実行中の場合に発生します。`thor gateway` が 1 つだけ実行されていることを確認してください。
